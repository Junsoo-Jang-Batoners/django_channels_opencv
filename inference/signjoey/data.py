...
def load_inference(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    """
    Load train, dev and optionally test data as specified in configuration.
    Vocabularies are created from the training set with a limit of `voc_limit`
    tokens and a minimum token frequency of `voc_min_freq`
...
# for inference repeatdata
def load_inference_data(data_cfg: dict, inf_data_name) -> (Dataset):
    """
    Load train, dev and optionally test data as specified in configuration.
    Vocabularies are created from the training set with a limit of `voc_limit`
    tokens and a minimum token frequency of `voc_min_freq`
    ...
    if isinstance(data_cfg["train"], list):
        train_paths = [os.path.join(data_path, x) for x in data_cfg["train"]]
        dev_paths = [os.path.join(data_path, x) for x in inf_data_name]
        # test_paths = [os.path.join(data_path, x) for x in data_cfg["test"]]
        pad_feature_size = sum(data_cfg["feature_size"])

    else:
        train_paths = os.path.join(data_path, data_cfg["train"])
        dev_paths = os.path.join(data_path, inf_data_name)
        # test_paths = os.path.join(data_path, data_cfg["test"])
        pad_feature_size = data_cfg["feature_size"]
    ...
    # print(f'train data load 전까지의 시간: {end_time - start_time}')
    ...
    # print(f'train data load 시간: {end_time - start_time}')
    ...
    # print(txt_max_size)
    # print(txt_min_freq)
    # print(gls_vocab_file)
    # print(txt_vocab_file)
    ...
    # print(f'inf data load 전까지의 시간: {end_time - start_time}')
    ...
    # print(f'inf data load 시간: {end_time - start_time}')
    ...
    # end_time = time.time()
    # print(f'소요시간 시간: {end_time - start_time}')
    return dev_data
